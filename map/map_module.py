...
class ProbabilisticTaskMapPlanner(PheromoneWaypointPlanner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.last_belief_entropy_normalized = 0.0
        self.last_belief_peak_probability = 0.0
        self.last_valid_belief_cell_count = 0
    ...
    def _reset_gated_belief_diagnostics(self):
        self.last_gated_belief_time_factor = 0.0
        self.last_gated_belief_entropy_confidence = 0.0
        self.last_gated_belief_total_confidence = 0.0
        self.last_gated_belief_uniform_mix = 0.0
        self.last_gated_belief_effective_weight = 0.0
        self.last_gated_belief_mix_entropy = 0.0
        self.last_gated_belief_mix_peak_probability = 0.0

    def gated_belief_distribution(self, step=None, normalized_entropy=None):
        """Return deterministic gated diagnostics and a normalized mixed belief."""
        step = self.runtime_step if step is None else int(step)
        entropy = (
            float(self.belief_entropy_normalized().item())
            if normalized_entropy is None else float(normalized_entropy)
        )
        time_denominator = max(
            self.pse_belief_gate_full_step - self.pse_belief_gate_start_step, 1
        )
        time_factor = float(min(1.0, max(
            0.0, (step - self.pse_belief_gate_start_step) / time_denominator
        )))
        entropy_denominator = max(
            self.pse_belief_entropy_high - self.pse_belief_entropy_low, self.eps
        )
        entropy_confidence = float(min(1.0, max(
            0.0, (self.pse_belief_entropy_high - entropy) / entropy_denominator
        )))
        total_confidence = time_factor * entropy_confidence
        uniform_mix = self.pse_belief_uniform_mix_high - total_confidence * (
            self.pse_belief_uniform_mix_high - self.pse_belief_uniform_mix_low
        )
        uniform_mix = float(min(
            self.pse_belief_uniform_mix_high,
            max(self.pse_belief_uniform_mix_low, uniform_mix),
        ))
        effective_weight = float(min(
            self.pse_belief_weight_max,
            max(0.0, self.pse_belief_weight_max * total_confidence),
        ))
        valid = self.flat_valid_mask_float
        raw = torch.clamp(
            torch.nan_to_num(self.belief_map.reshape(-1), nan=0.0), min=0.0
        ) * valid
        raw = raw / raw.sum().clamp_min(self.eps)
        uniform = valid / valid.sum().clamp_min(1.0)
        mixed = (1.0 - uniform_mix) * raw + uniform_mix * uniform
        mixed = mixed * valid
        mixed = mixed / mixed.sum().clamp_min(self.eps)
        positive = mixed[mixed > 0]
        mix_entropy = float(
            (-(positive * torch.log(positive)).sum() /
             math.log(max(2, self.valid_belief_cell_count()))).item()
        ) if positive.numel() else 0.0
        self.last_gated_belief_time_factor = time_factor
        self.last_gated_belief_entropy_confidence = entropy_confidence
        self.last_gated_belief_total_confidence = total_confidence
        self.last_gated_belief_uniform_mix = uniform_mix
        self.last_gated_belief_effective_weight = effective_weight
        self.last_gated_belief_mix_entropy = mix_entropy
        self.last_gated_belief_mix_peak_probability = float(mixed.max().item())
        return mixed, effective_weight
    ...
    def valid_belief_cell_count(self):
        if hasattr(self, "flat_valid_mask"):
            return int(torch.count_nonzero(self.flat_valid_mask).item())
        if hasattr(self, "belief_map"):
            return int(self.belief_map.numel())
        return int(math.prod(self.grid_size))

    def _valid_belief_probabilities(self):
        if not hasattr(self, "belief_map"):
            return torch.zeros((0,), dtype=self.dtype, device=self.device)
        probs = torch.nan_to_num(
            self.belief_map.reshape(-1), nan=0.0, posinf=0.0, neginf=0.0
        ).clamp_min(0.0)
        if hasattr(self, "flat_valid_mask"):
            probs = probs[self.flat_valid_mask]
        total = probs.sum()
        if probs.numel() == 0 or float(total.item()) <= self.eps:
            return torch.zeros_like(probs)
        return probs / total.clamp_min(self.eps)

    def belief_entropy_normalized(self):
        n_valid = self.valid_belief_cell_count()
        if n_valid <= 1:
            return torch.zeros((), dtype=self.dtype, device=self.device)
        probs = self._valid_belief_probabilities()
        positive = probs > 0.0
        if not torch.any(positive):
            return torch.zeros((), dtype=self.dtype, device=self.device)
        entropy = -(
            probs[positive] * torch.log(probs[positive].clamp_min(self.eps))
        ).sum()
        normalized = entropy / math.log(float(n_valid))
        return torch.clamp(
            torch.nan_to_num(normalized, nan=0.0, posinf=1.0, neginf=0.0),
            0.0,
            1.0,
        )

    def belief_peak_probability(self):
        probs = self._valid_belief_probabilities()
        if probs.numel() == 0:
            return torch.zeros((), dtype=self.dtype, device=self.device)
        return torch.clamp(probs.max(), 0.0, 1.0)

    def _sync_belief_diagnostics(self):
        self.last_belief_entropy = float(self.belief_entropy().item())
        self.last_belief_entropy_normalized = float(
            self.belief_entropy_normalized().item()
        )
        self.last_belief_peak_probability = float(
            self.belief_peak_probability().item()
        )
        self.last_valid_belief_cell_count = self.valid_belief_cell_count()
    ...
    def topk_belief_points(self, k=None):
        """Return a deterministic, probability-weighted spatial support set.

        Direct ``torch.topk`` is unsuitable when many cells have equal belief:
        its index-based tie handling can place every support point in one grid
        slice.  Systematic resampling preserves the belief mass while spreading
        a uniform belief over the complete valid grid.  Repeated samples from a
        high-probability cell are aggregated into one point and one probability.
        """
        if not hasattr(self, "belief_map"):
            self.reset_belief_map()
        requested = int(max(1, self.pse_belief_topk if k is None else k))
        flat = torch.nan_to_num(
            self.belief_map.reshape(-1), nan=0.0, posinf=0.0, neginf=0.0
        ).clamp_min(0.0)
        if hasattr(self, "flat_valid_mask"):
            valid_indices = torch.nonzero(self.flat_valid_mask, as_tuple=False).flatten()
        else:
            valid_indices = torch.arange(flat.numel(), device=self.device)
        ...
        probabilities = flat[valid_indices]
        total = probabilities.sum()
        if (not bool(torch.isfinite(total).item())) or float(total.item()) <= self.eps:
            probabilities = torch.full_like(
                probabilities, 1.0 / float(valid_indices.numel())
            )
        else:
            probabilities = probabilities / total.clamp_min(self.eps)

        sample_count = min(requested, int(valid_indices.numel()))
        if sample_count == int(valid_indices.numel()):
            return self.flat_xyz_centers[valid_indices], probabilities

        cdf = torch.cumsum(probabilities, dim=0)
        positions = (
            torch.arange(sample_count, dtype=self.dtype, device=self.device) + 0.5
        ) / float(sample_count)
        selected_local = torch.searchsorted(cdf, positions, right=False)
        selected_local = selected_local.clamp_max(valid_indices.numel() - 1)
        selected_global = valid_indices[selected_local]
        unique_global, counts = torch.unique(
            selected_global, sorted=True, return_counts=True
        )
        support_probabilities = counts.to(self.dtype) / float(sample_count)
        return self.flat_xyz_centers[unique_global], support_probabilities
    ...
    def expected_response_cost(self, standby_point):
        """Expected executor travel time under the deterministic belief support."""
        support, probabilities = self.topk_belief_points(self.pse_standby_topk)
        if support.numel() == 0:
            raise RuntimeError("belief support is empty while computing standby response cost")
        standby = self._as_points(standby_point).reshape(3)
        times = self.estimate_travel_time(standby, support, role="executor")
        return float(torch.sum(times * probabilities).item())
    ...
    def plan_executor_standby(self, executor_pos, prev_standby=None, move_weight=None, hysteresis_weight=None):
        ...
        try:
            ...
            if valid_pts is None or valid_pts.numel() == 0:
                valid_pts = (
                    self.flat_xyz_centers[self.flat_valid_mask]
                    if hasattr(self, "flat_valid_mask")
                    else self.flat_xyz_centers
                )
            if valid_pts.numel() == 0 or support.numel() == 0:
                return fallback.clone()

            # Deterministic stratified candidates avoid consuming the planner RNG.
            # This keeps paired ablations aligned when standby is disabled in one arm.
            cand_k = min(self.pse_standby_candidates, int(valid_pts.shape[0]))
            if cand_k == int(valid_pts.shape[0]):
                candidates = valid_pts
            else:
                candidate_indices = torch.floor(
                    (torch.arange(cand_k, device=self.device, dtype=self.dtype) + 0.5)
                    * float(valid_pts.shape[0])
                    / float(cand_k)
                ).to(torch.long)
                candidates = valid_pts[candidate_indices]
            ...
            # Vectorized expected response time for all candidate/support pairs.
            delta = support.unsqueeze(0) - candidates.unsqueeze(1)
            d_xy = torch.linalg.vector_norm(delta[..., :2], dim=-1)
            d_z = torch.abs(delta[..., 2])
            travel_times = d_xy / 1.15 + 0.6 * d_z / 0.60
            expected_response = torch.sum(travel_times * probs.unsqueeze(0), dim=1)
            ...
            hysteresis_w = (
                ...
                if hysteresis_weight is None
                else float(hysteresis_weight)
            )
            total = (
                expected_response
                + move_w * move_penalty
            ...
            chosen = torch.clamp(
                candidates[idx], min=torch.zeros_like(self.space_size), max=self.space_size
            )
            self.last_executor_standby = chosen.detach().clone()
            self.last_exec_response_cost = float(
                torch.nan_to_num(expected_response[idx], nan=0.0).item()
            )
            return chosen.clone()
        except (RuntimeError, ValueError, IndexError) as exc:
            raise RuntimeError(
                "executor standby planning failed "
                f"(executor={executor.detach().cpu().tolist()}, "
                f"previous={fallback.detach().cpu().tolist()})"
            ) from exc
